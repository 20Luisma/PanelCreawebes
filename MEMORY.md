...
- **Implementación de `.env` y Reestructuración de Carpetas (Fase 18 completada):**
  ...
  - **Documentación y Despliegue CI/CD:** Se rescrituró por completo el archivo `README.md` destacando todas las características técnicas, la implementación Clean Architecture y las directivas de automatización IA del proyecto. Se inicializó Git y se realizó un push remoto a la rama `main` del repositorio `20Luisma/PanelCreawebes` en GitHub.
- **Estado actual:** Proyecto 100% migrado a Clean Architecture. Estructura limpia en la raíz. Credenciales en `.env`. Todas las interfaces pulidas y alineadas. Repositorio de GitHub sincronizado exitosamente con la versión "Ferrari". Listo para deploy en `contenido.creawebes.com` (URLs relativas aseguran compatibilidad automática).

### 2026-03-26
- **Despliegue Diferencial a Producción Hostinger:** Búsqueda de credenciales y conexión exitosa por SSH. Creación de backup local en el server y despliegue quirúrgico del panel con `rsync` hacia `domains/contenido.creawebes.com/public_html/`. Se aislaron intactos los datos de clientes mediante listas negras paramétricas (`--exclude`).
- **Fix Crítico de Concurrencia PHP (Bug 504 Timeout):** Detección y contención de deadlock de sesión que congelaba la pasarela de entrada. Se inyectó el fix `session_write_close()` en 6 endpoints pesados paralelos (`api_carpetas`, `buscar`, `backup_proc`, `comprimir`, `descomprimir`, `download`), restableciendo 100% la concurrencia nativa.
- **Validación UX en Producción:** Verificación exitosa del nuevo diseño y UI en Hostinger, finalizando el plan maestro de migración de la Ferrari a entorno host.
- **Arquitectura de Backups por Lista Blanca:** Refactorización total de `BackupService.php`. Se eliminó el método de exclusiones y se implementó un escaneo estricto por lista blanca (`src/`, `assets/`, `*.php`, `*.json`) para ignorar masivamente carpetas de clientes. Ahora los backups de sistema pesan kilobytes y se completan en milisegundos.
- **Controles Avanzados de Backups UX:** Se inyectó lógicas en `backup.php` y `restaurar.php` añadiendo un botón para *Abortar* backups colgados en curso, y otro para *Eliminar* zips viejos directamente de la tabla visual.
- **Bugfix Notificaciones Chat Offline:** Diagnóstico de falla en `panel.js` donde el popup no se cerraba debido a que la cuenta de hijos en el DOM consideraba erróneamente el `<h3>` fijo del encabezado. La validación matemática de `.childElementCount === 0` nunca ocurría. Se refactorizó usando `querySelectorAll('.popup-mensaje').length === 0` eliminando el fallo zombie y configurando cierre automático por onclick a las burbujas locales.
- **Implementación CI/CD (Despliegue Continuo) y GitHub Actions:** Se creó y probó con éxito el workflow `deploy.yml`. GitHub asume ahora el mando y ejecuta el despliegue automático hacia los servidores de Hostinger a través de la herramienta `rsync` post validación.
- **Pila de Pruebas Automáticas Unitarias con PHPUnit:** Creación del set up base de Composer como controlador de dependencias dev (`composer.json`, `phpunit.xml` test suite, `.gitignore` optimizado). Elaboración del primer ensayo clínico en software `PathSecurityServiceTest` validando protección a prueba de balas contra vulnerabilidad transversal de directorios `../`. Inyección del proceso en el script de GitHub logrando la interrupción del CI/CD automática en fallas.
